...
let Engine, Render, Runner, Events, MouseConstraint, Vertices, Mouse, World, Bodies, Body, Composite, engine, render, runner, holderBodies, circleElems, curCirclePositions, dividerExtensions, xyPositionsCircle;
    ...
    event.pairs.forEach((x) => {
      if (x.bodyA.id.includes("circle") && x.bodyB.id.includes("circle")) {
        ...
        if (ExportedObject.checkSolution(arrangedArr)) {
          ...
          if (ExportedObject.solutionStep === ExportedObject.solutionLength) {
            messageElem.textContent = `You're done! It took ${
