...
function moveOrigin(originArr, posArr) {
    ...
    for (let i = 0; i < originArr.length; i++) {
        ...
        Body.translate(originArr[i], { x: posArr[i].x - originArr[i].position.x, y: posArr[i].y - originArr[i].position.y })
        Body.applyForce(originArr[i], originArr[i].position, {x:-originArr[i].force.x, y: -originArr[i].force.y})
    }
...
for (let i = 0; i < numArrayValues; i++) {
    ...
    */
    ...
    let tempCircle = Bodies.circle(xPos, yPos, docWidth / (numArrayValues + 1) / 4, {
        render: {
            sprite: {
    ...
    event.pairs.forEach(x=>{
        if(x.bodyA.id.includes('circle') && x.bodyB.id.includes('circle')){
            ...
            for(let i = 0; i< numArrayValues;i++){
                ...
                arrangedArr.push(Number(curCirclePositions[i].id.replace('circle','')));
                }
            ...
            moveOrigin([curCirclePositions[bodyAIndex],curCirclePositions[bodyBIndex]], [xyPositionsCircle[bodyAIndex],xyPositionsCircle[bodyBIndex]]);
            
            
        }

        //x.bodyA.id.includes('circle') && x.bodyB.id.includes('circle') ? :null;
    })
})
